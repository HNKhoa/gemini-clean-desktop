...
3. **`package.bat` tạo bản standalone ĐẦY ĐỦ** (thư mục `release\win-unpacked\`, **~370MB**): gói AI inpaint (CPU onnxruntime) + Thêm watermark (numpy/Pillow) — chạy trên máy **không cài gì**. (Trước đây portable chỉ có Xoá watermark.) **ffmpeg (~100MB) và model LaMa (~88MB) KHÔNG nhúng — máy khách tự tải lần đầu** (cần internet 1 lần; `ensure_ffmpeg()` + `_download_model()`, đều pin SHA-256, lưu ở `~/.gemini-clean/`). AI inpaint chạy CPU; muốn GPU NVIDIA thì bản nguồn + `setup-gpu.bat`. Xoá watermark chạy offline ngay.
...
| `package.bat` | Build **standalone ĐẦY ĐỦ** (folder `release\win-unpacked\`, **~370MB**, zip để gửi). Tạo venv build riêng (CPU onnxruntime → không đụng CUDA global), PyInstaller `--onedir` gói numpy/Pillow/watermark/lama_video/onnxruntime + `--add-data` alpha (đường dẫn TUYỆT ĐỐI), rồi `electron-builder --win dir`. **KHÔNG nhúng ffmpeg** (máy khách tự tải lần đầu). Có finder Python. |
...
- **ffmpeg tự tải lần đầu** (`ensure_ffmpeg()` trong server.py): nếu `shutil.which` không thấy → tải zip gyan.dev đã pin (URL + `FFMPEG_SHA256`) về `~/.gemini-clean/bin`, giải nén ffmpeg.exe/ffprobe.exe, **prepend vào `os.environ["PATH"]`** (để watermark/lama_video dùng được). Gọi ở đầu `_run_wm_job` và `_run_ai_job` (stage `ffmpeg`, có khoá `_ffmpeg_lock`). `_ai_available`/`_wm_available` dùng `_ffmpeg_obtainable()` (Windows luôn obtainable) nên KHÔNG còn bắt buộc ffmpeg sẵn. Lợi cho cả bản nguồn: không cần cài ffmpeg thủ công.
